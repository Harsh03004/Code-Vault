#include "aes.h"

// AES S-box
static const uint8_t sbox[256] = {
    
};

// AES inverse S-box
static const uint8_t inv_sbox[256] = {
    
};

// Round constant
static const uint8_t rcon[11] = {
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

AES::AES(const uint8_t* key) {
    key_expansion(key);
}

uint8_t AES::gmul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        bool hi_bit_set = (a & 0x80);
        a <<= 1;
        if (hi_bit_set) a ^= 0x1b;
        b >>= 1;
    }
    return p;
}

void AES::key_expansion(const uint8_t* key) {
    uint8_t temp[4];
    int i = 0;
    
    // First round key is the key itself
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            round_keys[0][r][c] = key[r + 4 * c];
        }
    }
    
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            round_keys[1][r][c] = key[16 + r + 4 * c];
        }
    }
    
    // Generate remaining round keys
    for (int round = 2; round <= ROUNDS; round++) {
        // RotWord and SubWord on last column of previous key
        temp[0] = sbox[round_keys[round-1][1][3]];
        temp[1] = sbox[round_keys[round-1][2][3]];
        temp[2] = sbox[round_keys[round-1][3][3]];
        temp[3] = sbox[round_keys[round-1][0][3]];
        
        // XOR with rcon
        temp[0] ^= rcon[round/2];
        
        // Generate first column
        for (int r = 0; r < 4; r++) {
            round_keys[round][r][0] = round_keys[round-2][r][0] ^ temp[r];
        }
        
        // Generate remaining columns
        for (int c = 1; c < 4; c++) {
            for (int r = 0; r < 4; r++) {
                round_keys[round][r][c] = round_keys[round][r][c-1] ^ round_keys[round-2][r][c];
            }
        }
    }
}

void AES::add_round_key(uint8_t state[4][4], int round) {
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            state[r][c] ^= round_keys[round][r][c];
        }
    }
}

void AES::sub_bytes(uint8_t state[4][4]) {
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            state[r][c] = sbox[state[r][c]];
        }
    }
}

void AES::inv_sub_bytes(uint8_t state[4][4]) {
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            state[r][c] = inv_sbox[state[r][c]];
        }
    }
}

void AES::shift_rows(uint8_t state[4][4]) {
    uint8_t temp;
    
    // Row 1: shift left by 1
    temp = state[1][0];
    state[1][0] = state[1][1];
    state[1][1] = state[1][2];
    state[1][2] = state[1][3];
    state[1][3] = temp;
    
    // Row 2: shift left by 2
    temp = state[2][0];
    state[2][0] = state[2][2];
    state[2][2] = temp;
    temp = state[2][1];
    state[2][1] = state[2][3];
    state[2][3] = temp;
    
    // Row 3: shift left by 3 (or right by 1)
    temp = state[3][3];
    state[3][3] = state[3][2];
    state[3][2] = state[3][1];
    state[3][1] = state[3][0];
    state[3][0] = temp;
}

void AES::inv_shift_rows(uint8_t state[4][4]) {
    uint8_t temp;
    
    // Row 1: shift right by 1
    temp = state[1][3];
    state[1][3] = state[1][2];
    state[1][2] = state[1][1];
    state[1][1] = state[1][0];
    state[1][0] = temp;
    
    // Row 2: shift right by 2
    temp = state[2][0];
    state[2][0] = state[2][2];
    state[2][2] = temp;
    temp = state[2][1];
    state[2][1] = state[2][3];
    state[2][3] = temp;
    
    // Row 3: shift right by 3 (or left by 1)
    temp = state[3][0];
    state[3][0] = state[3][1];
    state[3][1] = state[3][2];
    state[3][2] = state[3][3];
    state[3][3] = temp;
}

void AES::mix_columns(uint8_t state[4][4]) {
    uint8_t temp[4];
    
    for (int c = 0; c < 4; c++) {
        temp[0] = gmul(state[0][c], 2) ^ gmul(state[1][c], 3) ^ state[2][c] ^ state[3][c];
        temp[1] = state[0][c] ^ gmul(state[1][c], 2) ^ gmul(state[2][c], 3) ^ state[3][c];
        temp[2] = state[0][c] ^ state[1][c] ^ gmul(state[2][c], 2) ^ gmul(state[3][c], 3);
        temp[3] = gmul(state[0][c], 3) ^ state[1][c] ^ state[2][c] ^ gmul(state[3][c], 2);
        
        for (int r = 0; r < 4; r++) {
            state[r][c] = temp[r];
        }
    }
}

void AES::inv_mix_columns(uint8_t state[4][4]) {
    uint8_t temp[4];
    
    for (int c = 0; c < 4; c++) {
        temp[0] = gmul(state[0][c], 14) ^ gmul(state[1][c], 11) ^ gmul(state[2][c], 13) ^ gmul(state[3][c], 9);
        temp[1] = gmul(state[0][c], 9) ^ gmul(state[1][c], 14) ^ gmul(state[2][c], 11) ^ gmul(state[3][c], 13);
        temp[2] = gmul(state[0][c], 13) ^ gmul(state[1][c], 9) ^ gmul(state[2][c], 14) ^ gmul(state[3][c], 11);
        temp[3] = gmul(state[0][c], 11) ^ gmul(state[1][c], 13) ^ gmul(state[2][c], 9) ^ gmul(state[3][c], 14);
        
        for (int r = 0; r < 4; r++) {
            state[r][c] = temp[r];
        }
    }
}

void AES::encrypt_block(const uint8_t* input, uint8_t* output) {
    uint8_t state[4][4];
    
    // Copy input to state
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            state[r][c] = input[r + 4 * c];
        }
    }
    
    // Initial round
    add_round_key(state, 0);
    
    // Main rounds
    for (int round = 1; round < ROUNDS; round++) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, round);
    }
    
    // Final round (no mix columns)
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, ROUNDS);
    
    // Copy state to output
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            output[r + 4 * c] = state[r][c];
        }
    }
}

void AES::decrypt_block(const uint8_t* input, uint8_t* output) {
    uint8_t state[4][4];
    
    // Copy input to state
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            state[r][c] = input[r + 4 * c];
        }
    }
    
    // Initial round
    add_round_key(state, ROUNDS);
    
    // Main rounds
    for (int round = ROUNDS - 1; round > 0; round--) {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, round);
        inv_mix_columns(state);
    }
    
    // Final round (no mix columns)
    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(state, 0);
    
    // Copy state to output
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            output[r + 4 * c] = state[r][c];
        }
    }
}
